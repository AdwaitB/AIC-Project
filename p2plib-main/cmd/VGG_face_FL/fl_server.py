...
class GlobalModel(object):
    ...
    def update_weights(self, client_weights, client_sizes):
        ...
        print()
        print(f"number of clients contacted = {total_size}")
        for c in range(len(client_weights)):
            for i in range(len(new_weights)):
                new_weights[i] += client_weights[c][i] * 1 / len(client_weights)
        ...
        def handle_client_update(data):
            ...
            if data['round_number'] == self.current_round:
                ...
                if len(self.current_round_client_updates) > FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND * .7:
                    ...
                    print(FLServer.NUM_CLIENTS_CONTACTED_PER_ROUND)
                    # print(self.current_round_client_updates[0])
...
if __name__ == '__main__':
    server = FLServer(GlobalModel_VGG, sys.argv[1], sys.argv[2])
