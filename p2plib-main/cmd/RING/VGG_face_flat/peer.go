...
func IncreaseNumClientReady(){
	...
	if num_client_ready == (overlay.GetTotMembers()+1) {
	    ...
	    mutex_client_ready.Unlock()
            ...
            overlay.Report_GR(node.ID(), usermsg.Marshal(), num_client_ready)
...
func IncreaseNumClientUpdate(){
	...
	if num_client_update == (overlay.GetTotMembers()+1) {
	   ...
	   mutex_client_update.Unlock()
           ...
           C.call_c_func2(callbacks.on_reportclientupdate, (C.int)(num_client_update))
        }else{
	   mutex_client_update.Unlock()
	}
}
...
func IncreaseNumClientEvalInitiator(){
	...
	if num_client_eval == (overlay.GetTotMembers()+1) {
            // reset the value after reporting
	    num_client_eval = 0
            //fmt.Printf("reset current: %d, total expected : %d\n",num_client_update, overlay.GetTotMembers())
	    mutex_client_eval.Unlock()
...
func IncreaseNumClientEval(){
	...
	if num_client_eval == (overlay.GetTotMembers()+1) {
	    ...
	    mutex_client_eval.Unlock()
            ...
            // python code will aggreagte and report
            C.call_c_func2(callbacks.on_reportclienteval, (C.int)(num_client_eval))
        }else{
	    mutex_client_eval.Unlock()
	}
}
...
func on_request_group_fl_server(msg dual.P2pMessage) {
    //fmt.Printf("OP_REQUEST_GROUP. I am FL Server.\n")
    ...
    //Here, it is max # of clients that FL server can take
    overlay.SetGroupSize(group_size)
    overlay.SetMaxNeighborsGR(group_size)

    // set total # of clients for aggregation
    overlay.SetTotMembers(len(sub_list)-1) // I am expecting all workers to report me except myself

    // register members in application routing table
    // case A : if list -1 <= table_size : all member are workers in one level tree
    if( len(sub_list)-1 <= group_size ){
        //fmt.Printf("Case A len_list : %d table size : %d\n",len(sub_list)-1, p.MaxNeighborsGR())
        for _, v := range sub_list {
            if( v.Key == node.ID().ID.String()){
                //fmt.Printf("Case A : skip myself\n")
                continue // skip myself
            }
            msgctx, cancel := context.WithCancel(context.Background())
            err := node.SendMessage(msgctx, v.Addr, dual.P2pMessage{Opcode: byte(dual.OP_REQUEST_JOIN)}, dual.OP_GROUP_MULTICAST)
            /*
    ...
    // TODO
    // case B : if list -1 > table_size : every round FL server selects different members than previous round.(e.g. random or ones that did not participate)
}

func on_request_join_worker(msg dual.P2pMessage) {
    //fmt.Printf("OP_REQUEST_JOIN. I am a Worker.\n")

    // send report
    msgctx, cancel := context.WithCancel(context.Background())
    err := node.SendMessage(msgctx, overlay.GetLeader(), dual.P2pMessage{Aggregation: uint32(1), Opcode: byte(dual.OP_JOIN_GROUP)}, dual.OP_JOIN_GROUP)
    /*
    ...
    */
    ...
    if err != nil {
        fmt.Printf("Failed to send message to %s. Skipping... [error: %s]\n",
                    overlay.GetLeader(),err)
    }
}

func on_join_group(msg dual.P2pMessage) {
    if overlay.GetID() == dual.ID_PUBLISHER {
        on_group_done(msg)
    }

    if overlay.GetID() == dual.ID_INITIATOR { // FL server
        //fmt.Printf("Receive OP_JOIN_GROUP as an initiator.\n")
        value := msg.Aggregation
        //fmt.Printf("OP_JOIN_GROUP value : %d\n",value)

        overlay.Lock()
	overlay.SetNumMembers(overlay.GetNumMembers() + int(value))
        //fmt.Printf("OP_JOIN_GROUP num_members : %d, total expected : %d\n",overlay.GetNumMembers(), overlay.GetTotMembers())
        if overlay.GetNumMembers() == overlay.GetTotMembers() {
	    overlay.SetReport(true)
        }

        // send upper leader if it meets the total
        if(overlay.GetReport()){
            fmt.Printf("Report to publisher(%s) include me. num_members : %d+1, total expected : %d+1\n",overlay.GetLeader(),overlay.GetNumMembers(), overlay.GetTotMembers())
	                msgctx, cancel := context.WithCancel(context.Background())
            err := node.SendMessage(msgctx, overlay.GetLeader(), dual.P2pMessage{Aggregation: uint32(overlay.GetNumMembers()+1), Opcode: byte(dual.OP_JOIN_GROUP), Contents: msg.Contents}, dual.OP_JOIN_GROUP)
            /*
            fmt.Printf("Send report to upper leader %s opcode: OP_JOIN_GROUP value : %d\n",
                        overlay.GetLeader(),overlay.GetNumMembers()+1)
            */
            cancel()
            if err != nil {
