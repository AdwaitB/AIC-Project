...
class LocalModel(object):
    ...
    def aggregate_train_loss_accuracy(self, client_losses, client_accuracies, client_sizes, cur_round):
        ...
        #return aggr_loss, aggr_accuraries, aggr_size
...
class FederatedClient(object):
    MIN_NUM_WORKERS = 0 #total from this branch. This will be set by grouping protocol
    MAX_NUM_ROUNDS = 3 #TODO : publisher will pass this. For now, hard coded here
    ROUNDS_BETWEEN_VALIDATIONS = 2
    MAX_DATASET_SIZE_KEPT = 1200
    def __init__(self, host, port, bootaddr, datasource):
        #self.global_model = None #obsolete. Local_model is updated when global model is shared
        self.local_model = None
        self.datasource = datasource()

        # for sum of direct level member
        self.ready_client_sids = set(); #obsolete
        self.wakeup_client_sids = set(); #obsolete
        # for sum of lower level member
        self.lower_ready_clients  = 0 #obsolete
        self.lower_wakeup_clients  = 0 #obsolete
    ...
    def register_handles(self):
        ...
        def on_global_model(data):
            ...
            # initiator stores global model too
            #from keras.models import model_from_json
            #self.global_model = model_from_json(model_config['model_json'])
            ...
            metadata = {
                #'model_json': self.global_model.to_json(),
        ...
        def on_request_update_subleader(data):
            ...
            #forward REQUEST_UPDATE to lower clients is obsolete. This will be done in Go.
            #print("request updates to"+str(self.wakeup_client_sids)+'\n')
        ...
        def on_client_eval_done_initiator(arg):
            #aggr_test_loss, aggr_test_accuracy, aggr_test_size = self.global_model.aggregate_loss_accuracy(
        ...
        def on_train_next_round(arg):
            ...
            #client_sids_selected = random.sample(list(self.ready_client_sids), FederatedClient.MIN_NUM_WORKERS)
