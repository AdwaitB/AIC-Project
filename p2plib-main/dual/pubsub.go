...
func UnmarshalADMessage(buf []byte) (ADMessage, error) {
    pjtid := buf[0:34]
    pubid, err := p2plib.UnmarshalID(buf[34:])
    if err != nil {
...
type SubsMessage struct {
    //PjtID p2plib.ID //TODO: add pjt ID for multi project
    SubID p2plib.ID // ID is publickey
...
func UnmarshalSubsMessage(buf []byte) (SubsMessage, error) {
    subid, err := p2plib.UnmarshalID(buf)
    if err != nil {
...
func (ps *PubSub) NewProject(owner p2plib.ID) (string, *Project) {
    project := &Project{
                   Owner: owner,
		   PjtID: CreateUUID(),
