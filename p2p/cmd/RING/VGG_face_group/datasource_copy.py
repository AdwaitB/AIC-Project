...
class VggFace2(DataSource):
    ...
    def __init__(self):
        def read_data(tp="Train", N=1542865627584):
            target = pd.read_csv("./archive/{}-{}.csv".format(tp.title(), N))
            pt = pd.read_csv("./archive/{}_Beneficiarydata-{}.csv".format(tp.title(), N))
            in_pt = pd.read_csv("./archive/{}_Inpatientdata-{}.csv".format(tp.title(), N))
            out_pt = pd.read_csv("./archive/{}_Outpatientdata-{}.csv".format(tp.title(), N))
        ...
        data['N_unique_Physicians'] = N_unique_values(
            data[['AttendingPhysician', 'OperatingPhysician', 'OtherPhysician']])
        # We separate the types of physicians into numeric values
        data[['AttendingPhysician', 'OperatingPhysician', 'OtherPhysician']] = np.where(
            data[['AttendingPhysician', 'OperatingPhysician',
        ...
        data['Same_Physician'] = data.apply(
            lambda x: 1 if (x['N_unique_Physicians'] == 1 and x['N_Types_Physicians'] > 1) else 0, axis=1)
        ...
        data['Same_Physician2'] = data.apply(
            lambda x: 1 if (x['N_unique_Physicians'] == 2 and x['N_Types_Physicians'] > 2) else 0, axis=1)
        ...
        _sum = data.groupby(['Provider'], as_index=False)[
        ...
        _mean = data.groupby(['Provider'], as_index=False)[['NoOfMonths_PartACov', 'NoOfMonths_PartBCov',
                                                            ...
                                                            'ChronicCond_stroke', 'Dead', 'N_Procedure',
                                                            'N_Unique_Claims',
        ...
        df = _total[['InscClaimAmtReimbursed_sum', 'N_Extra_Claims_sum', 'Claim_Days_sum',
                     'AttendingPhysician_mean', 'Missing_Deductible_Amount_Paid_mean', 'Dead_mean', 'Claim_Days_mean',
                     'N_Extra_Claims_mean',
