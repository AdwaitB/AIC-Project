...
func(p *Protocol) markSeenRelay(target p2plib.ID, euuid []byte) {
    // unmarshal
    var uuid string
    err := json.Unmarshal(euuid, &uuid)
    if err != nil {
...
func(p *Protocol) hasSeenRelay(target p2plib.ID, euuid []byte) bool{
    // unmarshal
    var uuid string
    err := json.Unmarshal(euuid, &uuid)
    if err != nil {
...
func (p *Protocol) Find(target p2plib.PublicKey, isBootstrap bool, opts ...IteratorOption) []p2plib.ID {
...
func (p *Protocol) DiscoverLocal(isBootstrap bool, opts ...IteratorOption) []p2plib.ID {
	return p.Find(p.node.ID().ID, isBootstrap, opts...)
}

// Discover attempts to discover new peers to your node through peers your node  already knows about by calling
// the FIND_NODE S/Kademlia RPC call with your nodes ID.
func (p *Protocol) DiscoverRandom(isBootstrap bool, opts ...IteratorOption) []p2plib.ID {
...
func (p *Protocol) RemoveConn(table *Table, id p2plib.ID, isInbound bool) {
...
func (p *Protocol) ReplaceConn(id p2plib.ID, isInbound bool) {
...
func (p *Protocol) Ack_bc(id p2plib.ID, isInbound bool, opcode ...byte) {
...
func (p *Protocol) Broadcast_BC(sender p2plib.ID, data []byte, opcode byte) {
...
func (p *Protocol) Multicast_GR(sender p2plib.ID, data []byte, opcode byte) {
...
func (p *Protocol) Fedcomp_GR(sender p2plib.ID, data []byte) {
    for _, id := range p.Table_gr().KEntries(p.maxNeighborsGR) {
        if id.ID == sender.ID || id.Address == p.GetLeader(){
            //fmt.Printf("skip sender %v %v\n", id.ID.String()[:printedLength], sender.ID.String()[:printedLength])
            continue
...
func (p *Protocol) Report_GR(sender p2plib.ID, data []byte,  aggregation int) {
    ctx, cancel := context.WithCancel(context.Background())
...
func (p *Protocol) On_recv_gossip_push_only(msg GossipMessage, ctx p2plib.HandlerContext) {
...
func (p *Protocol) On_recv_gossip_push(msg GossipMessage, ctx p2plib.HandlerContext) {
    //fmt.Printf("onRecvGossipPush: OP_GOSSIP_PUSH RECEIVE ITEM count:%d\n",msg.Count)
...
func (p *Protocol) On_recv_gossip_pullreq(msg GossipMessage, ctx p2plib.HandlerContext) {
...
func (p *Protocol) On_recv_relay(relaymsg RelayMessage, ctx p2plib.HandlerContext) {
...
func (p *Protocol) Gossip_BC(uuid string, data []byte, avoid []string, msgtype byte, targetid p2plib.ID) {
    euuid, _ := json.Marshal(uuid)
    p.__Gossip_BC(euuid, data, avoid, msgtype, targetid)
}

func (p *Protocol) __Gossip_BC_push_only(euuid []byte, data []byte, avoid []string, msgtype byte, targetid p2plib.ID) {
...
func (p *Protocol) __Gossip_BC(euuid []byte, data []byte, avoid []string, msgtype byte, targetid p2plib.ID) {
    peers := p.Table_bc().KEntries(p.maxNeighborsBC)
    if len(avoid) > 0 {
...
func(p *Protocol) Sendmsg(euuid []byte, data []byte, length int, listdata []byte, id p2plib.ID, opcode byte, msgtype byte, targetid p2plib.ID) {
    ctx, cancel := context.WithCancel(context.Background())
...
func (p *Protocol) Remove(peers []p2plib.ID, entry []string) []p2plib.ID {
...
func (p *Protocol) FindRandMember(push int, len_peers int, peers []p2plib.ID) ([]p2plib.ID,[]p2plib.ID) {
...
func (p *Protocol) Choose(peers []p2plib.ID, entry []string) ([]p2plib.ID, []p2plib.ID) {
    var first []p2plib.ID
